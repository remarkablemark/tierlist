...
- [ ] T006 Define TypeScript types in `src/types/tierList.types.ts`: TierList, Tier, TierListItem, ItemMetadata, TierListSettings, TierListState, TierListAction
- [ ] T007 [P] Create utility: `src/utils/escapeHtml.ts` with escapeHtml function and tests
- [ ] T008 [P] Create utility: `src/utils/validation.ts` with validateTierList, validateTier, validateItem, isValidCssColor, getTotalItemCount, findItemLocation and tests
- [ ] T009 [P] Create utility: `src/utils/generateId.ts` with generateId function using crypto.randomUUID() and tests
- [ ] T010 Create IndexedDB service: `src/services/storage.ts` with openDB, saveTierList, loadTierList, deleteTierList, saveImage, loadImage, deleteImage functions
- [ ] T011 Write storage service tests: `src/services/storage.test.ts` mocking IndexedDB with fake-indexeddb
- [ ] T012 Create tier list reducer: `src/store/tierListReducer.ts` implementing all TierListAction types (TIER_ADD, TIER_DELETE, TIER_REORDER, TIER_UPDATE_LABEL, TIER_UPDATE_COLOR, TIER_RESET, ITEM_ADD, ITEM_DELETE, ITEM_MOVE, ITEM_REORDER, ITEM_UPDATE_LABEL, DRAG_START, DRAG_MOVE, DRAG_END, UNDO, REDO, SETTINGS_UPDATE, LOAD, SAVE_REQUEST, SAVE_SUCCESS, SAVE_ERROR)
- [ ] T013 Write reducer tests: `src/store/tierListReducer.test.ts` covering all action types
- [ ] T014 Create context provider: `src/store/tierListContext.tsx` with TierListContext, TierListProvider, useTierListContext
- [ ] T015 Write context tests: `src/store/tierListContext.test.tsx` testing provider and hook
...
- [ ] T025 [US1] Create Tier component: `src/components/Tier/Tier.tsx` with SortableContext, renders tier label, color, delete button, drop zone
- [ ] T026 [US1] Create Tier types: `src/components/Tier/Tier.types.ts` with TierProps interface
- [ ] T027 [US1] Create barrel export: `src/components/Tier/index.ts`
- [ ] T028 [US1] Create useTierList hook: `src/hooks/useTierList.ts` wrapping context with tier operations (addTier, deleteTier, reorderTiers, updateTierLabel, updateTierColor, resetTier)
- [ ] T029 [US1] Write useTierList hook tests: `src/hooks/useTierList.test.ts`
- [ ] T030 [US1] Add unassigned items area UI in TierList component for items from deleted tiers
...
- [ ] T035 [P] [US2] Create image upload utility: `src/utils/imageUpload.ts` with fileToBlob, blobToDataUrl functions and tests
- [ ] T036 [P] [US2] Create TierListItem component: `src/components/TierListItem/TierListItem.tsx` with SortableItem, renders image, label, drag handle
- [ ] T037 [US2] Create TierListItem types: `src/components/TierListItem/TierListItem.types.ts` with TierListItemComponentProps interface
- [ ] T038 [US2] Create barrel export: `src/components/TierListItem/index.ts`
- [ ] T039 [US2] Create AddItemButton component: `src/components/AddItemButton/AddItemButton.tsx` with file input, item count validation, warning at 50+ items
...
- [ ] T049 [P] [US3] Create color picker component: `src/components/ColorPicker/ColorPicker.tsx` with predefined color palette, custom color input
- [ ] T050 [US3] Create ColorPicker types: `src/components/ColorPicker/ColorPicker.types.ts`
- [ ] T051 [US3] Create barrel export: `src/components/ColorPicker/index.ts`
- [ ] T052 [US3] Add tier customization UI to Tier component: color picker button, label edit input, reset button
- [ ] T053 [US3] Extend useTierList hook with customization operations: updateTierColor, updateTierLabel, resetTier
- [ ] T054 [US3] Add tier customization actions to reducer: TIER_UPDATE_COLOR, TIER_UPDATE_LABEL, TIER_RESET with isCustomColor and isCustomLabel flags
...
- [ ] T074 [US5] Add export container ref for capturing tier list canvas at full resolution
...
- [ ] T075 [P] Unit test: Undo action restores previous state in `src/store/tierListReducer.test.ts`
- [ ] T076 [P] Unit test: Redo action restores future state in `src/store/tierListReducer.test.ts`
- [ ] T077 [P] Unit test: 50-action limit with circular buffer in `src/store/tierListReducer.test.ts`
- [ ] T078 [P] Integration test: Undo/redo UI updates canUndo/canRedo in `src/components/UndoRedoControls/UndoRedoControls.test.tsx`

### Implementation for Undo/Redo

- [ ] T079 Wrap TierListState with undo/redo wrapper: past array (max 50), present state, future array
- [ ] T080 Implement UNDO action in reducer: moves present to future, restores from past
- [ ] T081 Implement REDO action in reducer: moves present to past, restores from future
- [ ] T082 Implement circular buffer: enforce 50-action limit, clear future on new actions
- [ ] T083 Create UndoRedoControls component: `src/components/UndoRedoControls/UndoRedoControls.tsx` with undo/redo buttons, disabled states, keyboard shortcuts display
- [ ] T084 Create UndoRedoControls types: `src/components/UndoRedoControls/UndoRedoControls.types.ts` with UndoRedoControlsProps interface
- [ ] T085 Create barrel export: `src/components/UndoRedoControls/index.ts`
- [ ] T086 Extend useTierList hook with undo, redo operations and canUndo, canRedo computed values
...
- [ ] T090 [P] Configure @dnd-kit keyboard sensor in useTierList hook
- [ ] T091 [P] Configure @dnd-kit touch sensor with activation constraint (distance: 5)
- [ ] T092 Add ARIA labels to all interactive elements: TierList (role="application"), Tier (role="region"), TierListItem (role="listitem")
- [ ] T093 Add aria-grabbed to draggable items, aria-dropeffect to drop zones
- [ ] T094 Create live region for screen reader announcements during drag operations
- [ ] T095 Implement step-by-step audio cues: "Press Enter to pick up, arrow keys to move, Enter to drop"
- [ ] T096 Ensure 44x44px minimum touch targets via Tailwind classes (min-h-11 min-w-11)
- [ ] T097 Add visible focus indicators: focus:ring-2 focus:ring-offset-2 on all interactive elements
- [ ] T098 Add keyboard instructions to aria-describedby for draggable items
...
- [ ] T099 [P] Add responsive design: mobile-first Tailwind breakpoints (sm:, md:, lg:, xl:) for 320px-1920px viewport
- [ ] T100 [P] Implement dark mode support: system-preference only via prefers-color-scheme media query, dark: prefix classes
- [ ] T101 [P] Add performance warning: display toast when item count >= 50, hard stop at 100 items
- [ ] T102 [P] Add loading states: skeleton loaders for tier list, items, export
- [ ] T103 [P] Add error boundaries: component-level error boundaries with fallback UI
- [ ] T104 [P] Write integration tests: complete user journey from create to save to export
- [ ] T105 [P] Run test coverage: `npm run test:ci` and verify 100% coverage (excluding barrel exports)
- [ ] T106 [P] Run type check: `npm run lint:tsc` with zero errors
- [ ] T107 [P] Run linter: `npm run lint` with zero errors
- [ ] T108 [P] Update quickstart.md: validate all commands and paths work correctly
- [ ] T109 [P] Add TSDoc comments to all public APIs: components, hooks, utilities
- [ ] T110 [P] Code cleanup: remove unused imports, dead code, console statements
...
- T016, T017, T018, T019 (tests) can run in parallel
- T020 (createDefaultTierList) and T016-T019 (tests) can start in parallel

**User Story 2**:

- T031, T032, T033, T034 (tests) can run in parallel
- T035 (imageUpload) and T036 (TierListItem) can develop in parallel

**User Story 3**:

- T046, T047, T048 (tests) can run in parallel
- T049 (ColorPicker) and T052 (Tier customization UI) can develop in parallel
...
- T068 (exportToPng) and T070 (ExportButton) can develop in parallel

**Undo/Redo**:

- T075, T076, T077, T078 (tests) can run in parallel
...
- T099-T104 can implement in parallel
- T105-T110 (validation) should run sequentially at end
...
# Developer A: T020 createDefaultTierList.ts
# Developer B: T016-T019 tests
```
...
# Developer A: T035 imageUpload.ts
# Developer B: T036 TierListItem.tsx
# Developer C: T039 AddItemButton.tsx
```
...
2. Once Foundational is done:
   ...
   - Developer C: User Story 3 (T046-T054) + User Story 4 (T055-T065)
3. Stories complete and integrate independently
4. Team reconvenes for Undo/Redo (T075-T086)
5. Team reconvenes for Accessibility (T087-T098)
6. Team reconvenes for Polish (T099-T110)
...
- **User Story 1**: 15 tasks (T016-T030)
- **User Story 2**: 15 tasks (T031-T045)
- **User Story 3**: 9 tasks (T046-T054)
- **User Story 4**: 11 tasks (T055-T065)
- **User Story 5**: 10 tasks (T066-T074)

### Suggested MVP Scope

**Minimum Viable Product** (User Story 1 + User Story 2 only):

- T001-T045 (Setup + Foundational + US1 + US2)
